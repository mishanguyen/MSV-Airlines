...
function Flights() {
    ...
    useEffect(() => {
        async function fetchOrigins() {
            const response = await axios.get('http://localhost:8000/api/flights/origins');
    ...
    useEffect(() => {
        async function fetchDestinations() {
            const response = await axios.get('http://localhost:8000/api/flights/destinations');
    ...
    //     try {
    //         let url = `http://localhost:8000/api/flights/flights?origin=${origin}&destination=${destination}&departureDate=${departDate}`;
    //         if (isRoundTrip) {
    ...
    //             const returnUrl = `http://localhost:8000/api/flights/flights?origin=${destination}&destination=${origin}&departureDate=${returnDate}`;
    //             const returnResponse = await axios.get(returnUrl);
    //             setFlights(prevFlights => [...prevFlights, ...returnResponse.data.rows]);
    //         }
